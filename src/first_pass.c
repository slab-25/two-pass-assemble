...
static bool process_label(const char *label, symbol_table_t *symbols, int address, instruction_type_t type);
...
bool process_extern_directive(parsed_line_t *line, symbol_table_t *symbols) {
    ...
    if (!add_symbol(symbols, symbol_name, 0, SYMBOL_EXTERNAL)) {
        symbol_t *existing = find_symbol(symbols, symbol_name);
